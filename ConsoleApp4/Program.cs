...
namespace ConsoleApp4
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("Для создания изображений используйте: /img ваш запрос");
            ...
            while (true)
            {
                ...
                if (UserInput.StartsWith("/img", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        Console.WriteLine("Введите текст после /img");
                        continue;
                    }

                    await HandleImageCommand(Token, prompt);
                    continue;
                }
            }
        }
        ...
        public static async Task<string> GetPictureAndSave(string token, List<Request.Message> messages, string clientId = null)
        {
        ...
        private static async Task HandleImageCommand(string Token, string prompt)
        {
            string baseUrl = "https://gigachat.devices.sberbank.ru/api/v1";
                    ...
                    var imgMessages = new List<Request.Message>()
            {
                new Request.Message() { role = "user", content = prompt }
                    ...
                    string fileUrl = $"{baseUrl}/files/{firstFileId}/content";
                    ...
                    string outPath = Path.Combine(desktop, $"gigachat_{firstFileId}.jpg");
                    ...
                    Console.Write("\nУстановить это изображение как обои? (д/н): ");
